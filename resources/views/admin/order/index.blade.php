...
@section('content')
    <style>
        ...
        /* صف الـ child الخاص بالـ Responsive */
        #orders-table tbody tr.child {
            box-shadow: none;
        }

        #orders-table tbody tr.child td {
        ...
        #orders-table tbody tr.child ul.dtr-details {
            width: 100%;
        }

        #orders-table tbody tr.child ul.dtr-details li {
                ...
                <div class="modal-body px-4 py-4">
                    ...
                    <div class="p-3 rounded-4"
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-2 pb-2"
                            style="border-bottom: 1px dashed rgba(0,0,0,0.15);">
                            <span class="text-muted fw-semibold">
                                <i class="fas fa-tag me-1"></i> السعر الأصلي للمنتج
                            </span>
                            <span class="fw-bold"><span id="modalOriginalPrice"></span> JOD</span>
                        </div>

                        {{-- قيمة الخصم --}}
                        <div class="d-flex justify-content-between align-items-center mb-2 pb-2 text-danger"
                            style="border-bottom: 1px dashed rgba(0,0,0,0.15);">
                            <span class="fw-semibold">
                                <i class="fas fa-minus-circle me-1"></i>
                                قيمة الخصم (<span id="modalDiscountDisplay"></span>)
                            </span>
                            <span class="fw-bold">- <span id="modalDiscountAmount"></span> JOD</span>
                        </div>

                        {{-- السعر المُطبَّق --}}
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center">
                                <i class="fas fa-money-bill-wave text-warning me-2"
    ...
    <style>
        /* === Group Search Autocomplete Widget === */
        .group-search-wrapper {
            position: relative;
            margin-left: 12px;
        }

        .group-search-input-wrap {
            display: flex;
            align-items: center;
            gap: 6px;
            background: #fff;
        ...
        .group-search-input-wrap:focus-within {
            border-color: #6366f1;
            box-shadow: 0 0 0 3px rgba(99, 102, 241, .15);
        }

        .group-search-input-wrap i {
            color: #9ca3af;
            font-size: 12px;
            flex-shrink: 0;
        }

        .order-designer-select {
            min-width: 120px;
            max-width: 150px;
            font-size: 0.78rem;
        ...
        .group-dropdown-list li:hover,
        .group-dropdown-list li.active-highlight {
            background: #eef2ff;
            color: #4f46e5;
        }

        .group-dropdown-list li i {
            color: #a5b4fc;
            font-size: 11px;
        }
        ...
        .group-badge-active button:hover {
            color: #ef4444;
        }
    ...
    <script>
        $(document).ready(function () {
            ...
            // متغير اسم المجموعة المختارة (خارج DataTable حتى تقدر الـ ajax data function تقرأها)
            ...
            const table = $('#orders-table').DataTable({
                ...
                columns: [
                    ...
                    {
                        // عمود موحد: رقم الهاتف + رقم الهاتف 2
                        data: 'phone',
                        name: 'phone',
                        title: 'الهاتف',
                        orderable: false,
...

                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            if (data) {
                                return `<span>${parseFloat(data).toFixed(2)} JOD</span>`;
                            }
                        ...
                        render: function (data, type, row) {
                            ...
                            if (data.incomplete) {
                                return `<span class="badge-discount badge-group-warn js-show-group-warning" 
                                                                                        ...
                                                                                        data-original-price="${data.original_price ?? 0}"
                                                                                        data-discount-amount="${data.discount_amount ?? 0}"
                                                                                        data-discount-display="${data.discount_display ?? ''}"
                                                                                        data-applied-price="${data.applied_price ?? 0}"
                    ...
                    {
                        // عمود التحكم الخاص بالـ Responsive - يُضاف تلقائياً
                        data: null,
                        defaultContent: '',
                ...
                initComplete: function () {
                    ...
                    const groupSearchWidget = $(`
                                                                                                                <div class="group-search-wrapper" id="groupSearchWrapper">
                                                                                                                    <div class="group-search-input-wrap">
                                                                                                                        <i class="fas fa-users"></i>
                                                                                                                        <input type="text" id="groupSearchInput" placeholder="بحث عن مجموعة..." autocomplete="off" />
                                                                                                                        <button class="group-clear-btn" id="groupClearBtn" title="مسح"><i class="fas fa-times"></i></button>
                                                                                                                    </div>
                                                                                                                    <ul class="group-dropdown-list" id="groupDropdownList"></ul>
                                                                                                                </div>
                                                                                                            `);
                    ...
                    $('.dataTables_filter').append(groupSearchWidget);

                    // ---- Logic: Group Autocomplete ----
                    ...
                    function selectGroup(name) {
                        ...
                        const $badge = $(`
                                                                                                                    <span class="group-badge-active">
                                                                                                                        ...
                                                                                                                        ${name}
                                                                                                                        <button type="button" id="removeBadgeBtn"><i class="fas fa-times"></i></button>
                                                                                                                    </span>`);
                        $wrap.prepend($badge);
                    ...
                    function clearGroup() {
                        ...
                        const $wrap = $('#groupSearchWrapper .group-search-input-wrap');
                        $wrap.find('.group-badge-active').remove();
                        $('#groupSearchInput').val('').attr('placeholder', 'بحث عن مجموعة...').show();
                        $('#groupDropdownList').hide();
                        table.ajax.reload();
                    }
                    ...
                    // Override ajax data to use selectedGroupName
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            Swal.fire({
                                title: 'خطأ',
                                text: response.message || 'فشل تحديث حالة الطلب.',
                                icon: 'error',
                                confirmButtonColor: '#dc2626'
                            });
                        }
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (!response.success) {
                            Swal.fire({
                                title: 'خطأ',
                                text: response.message || 'فشل تحديث المصمم.',
                                icon: 'error',
                                confirmButtonColor: '#dc2626'
                            });
            ...
            @if(auth()->user()->isAdmin())
                ...
                $('#bulkDeleteBtn').on('click', function () {
                    ...
                    if (orderIds.length === 0) {
                        Swal.fire({
                            title: 'تنبيه',
                            text: 'لم يتم تحديد أي طلبات للحذف.',
                            icon: 'warning',
                            confirmButtonColor: '#f59e0b'
                        });
                        return;
            ...
            $(document).on('click', '.js-show-group-warning', function () {
                ...
                const discountDisplay = $(this).data('discount-display') || '';
                const appliedPrice = parseFloat($(this).data('applied-price')) || 0;
                ...
                $('#modalOriginalPrice').text(originalPrice.toFixed(2));
                $('#modalDiscountAmount').text(discountAmount.toFixed(2));
                $('#modalDiscountDisplay').text(discountDisplay);
                $('#modalAppliedPrice').text(appliedPrice.toFixed(2));
