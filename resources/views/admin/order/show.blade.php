...
@section('content')
                            ...
                            <div class="mb-4">
                                ...
                                @if ($hasSvg)
                                    ...
                                    <div class="svg-phrase-wrapper">
                                        {{-- أزرار فوق الصورة --}}
                                        <div class="svg-phrase-actions">
                                            <button type="button" class="btn btn-primary btn-sm copy-svg-button">
                                                <i class="fas fa-copy me-1"></i> نسخ للـ Illustrator
                                            </button>
                                            <button type="button"
                                                class="btn btn-outline-secondary btn-sm download-svg-phrase-btn">
                                                <i class="fas fa-download me-1"></i> تحميل .svg
                                            </button>
                                        </div>
                                        {{-- معاينة SVG --}}
                                        <div class="svg-phrase-preview">
                                            {!! $svgRecord->svg_code !!}
                                        </div>
                                    </div>
                                @elseif (!$svgTitle)
    ...
    <script>
        (function () {
            ...
            if (oldPane) {
                oldPane.classList.remove('show', 'active');
            }
                    ...
                    <div class="modal-body px-4 py-4">
                        ...
                        <div class="p-3 rounded-4"
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-2 pb-2"
                                ...
                                <span class="fw-bold">{{ number_format($groupWarning['original_price'], 2) }} JOD</span>
                            </div>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <h4 class="mb-0 fw-black text-dark" style="letter-spacing: -0.5px;">
                                    {{ number_format($groupWarning['applied_price'], 2) }}
                                    <span class="fs-6 text-muted">JOD</span>
                                </h4>
                            </div>
                        </div>
    ...
    <script>
        ...
        window.finalDesignSvg = @json($finalDesignSvg ?? '');
        window.decorationDesignSvg = @json($decorationDesignSvg ?? '');
    </script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.js-copy-decoration-svg-btn').forEach(function (btn) {
                btn.addEventListener('click', async function () {
                    ...
                    try {
                        await navigator.clipboard.writeText(window.decorationDesignSvg);
                        this.innerHTML = '<i class="fas fa-check me-1"></i> تم نسخ SVG!';
                        this.classList.replace('btn-dark', 'btn-success');
                    } catch (err) {
                        this.innerHTML = '<i class="fas fa-times me-1"></i> فشل النسخ';
                    ...
                    img.onload = () => {
                        ...
                        if (vbAttr) {
                            ...
                            if (parts.length === 4) {
                                ...
                                // scale to fit مع الحفاظ على النسبة — بدون قطع
                                const scale = Math.min(drawArea / vbW, drawArea / vbH);
                                svgW = vbW * scale;
                                svgH = vbH * scale;
                            }
                        }
                        const offsetX = (size - svgW) / 2;
                        const offsetY = (size - svgH) / 2;

                        ctx.drawImage(img, offsetX, offsetY, svgW, svgH);
    ...
    <script>
        ...
        document.querySelectorAll('.copy-svg-button').forEach(btn => {
            btn.addEventListener('click', async function () {
                ...
                try {
                    ...
                    this.innerHTML = '<i class="fas fa-check me-1"></i> تم! افتح Illustrator ثم Ctrl+V';
                    this.classList.replace('btn-primary', 'btn-success');
                } catch (err) {
                    console.error(err);
                    this.innerHTML = '<i class="fas fa-times me-1"></i> فشل — جرب زر التحميل';
        ...
        document.querySelectorAll('.download-svg-phrase-btn').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = 'book_phrase_order_{{ $order->id }}.svg';
                a.click();
